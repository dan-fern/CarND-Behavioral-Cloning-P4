...
class SimplePIController:
    ...
    def update( self, measurement ):
        ...
        return self.Kp * self.error + self.Ki * self.integral + self.Kd

    
controller = SimplePIController( 0.055, 0.001, -0.11 )
...
def telemetry(sid, data):
    if data:
        ...
        # Use if with speed-trained model
        controller.set_desired( float( model_t.predict( image_array[None, :, :, :], batch_size=1 ) ) )
        throttle = controller.update( float( speed ) )
        
        # Use if with thorottle-trained model
        #throttle = float( model_t.predict( image_array[None, :, :, :], batch_size=1 ) / 2 ) 
...
if __name__ == '__main__':
    ...
    model_t = load_model(args.model_t)
